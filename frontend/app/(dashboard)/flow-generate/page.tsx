...
export default function FlowGeneratePage() {
  ...
  function handleCreated(project: FlowProject) {
    setShowModal(false);
    router.push(`/flow-generate/${project.id}`);
  }

  return (
    <div className="space-y-5">
      <FlowBreadcrumb items={[{ label: 'Flow Generate' }]} />

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Flow Generate</h1>
          <p className="text-slate-500 text-sm mt-0.5">รายการโปรเจคที่เคยสร้างสำหรับ Flow Generate</p>
        </div>
        <button
      ...
      {loaded && projects.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center gap-3 bg-white rounded-2xl border border-slate-100">
          ...
          <h2 className="text-base font-semibold text-slate-700">ยังไม่มีโปรเจค</h2>
          <p className="text-slate-500 text-sm">เริ่มต้นสร้างโปรเจคแรกของคุณได้เลย</p>
          <button
            onClick={() => setShowModal(true)}
