'use client';
// Forked from components/flow-generate/ui-builder/BuilderCanvas.tsx: same drag-drop tree editor
// (dnd-kit sort/drop, AddControlDialog/AddTableDialog dialogs, Modal-hosted PropertiesPanel), but
// without the "drag a saved Component from the palette" merge path (flowUiStore-coupled, not
// something UIs Gen has in v1) and threading `targets` through to the action-step editor instead
// of `projectId`/`currentUiId`.
...
import { ActionTarget } from './ActionStepsEditor';
import AddControlDialog, { ControlPreset } from '@/components/flow-generate/ui-builder/AddControlDialog';
import AddTableDialog from '@/components/flow-generate/ui-builder/AddTableDialog';
import '@/components/flow-generate/ui-builder/dragCursor.css';
import {
  BuilderNode,
  BuilderElementType,
  addNodeAtPath,
  ...
  tryGetNodeAtPath,
  pathKey,
} from '@/components/flow-generate/ui-builder/treeOps';
...
export default function BuilderCanvas({ schema, uiSchema, targets, onSchemaChange, onUiSchemaChange }: BuilderCanvasProps) {
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    if (fromContainerPath.join('-') !== toContainerPath.join('-')) return;
